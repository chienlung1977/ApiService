...
exports.remove = function(model,_primarycontactnuber,repsonse)
{
...
exports.update = function (model, requestBody, response) {
	...
	+ ' does not exist. The contact will be created.');
	 ...
	 if (!error) {
	 console.log('Successfully updated contact with primary
	number: '+ primarynumber);
	 ...
	 exports.create = function (model, requestBody, response) {
		 ...
		 contact.save();	 
			 ...
			 console.log('Checking if contact saving failed due to
			already existing primary number:' + primarynumber);
			 model.findOne({primarycontactnumber: primarynumber},
			 function(error, data) {
			 if (error) {
			 console.log(error);
			 ...
			 var contact = toContact(requestBody, model);
			 if (!data) {
			 console.log('The contact does not exist. It will be
			created');
			 contact.save(function(error) {
			 if (!error) {
			 contact.save();
			 ...
			 console.log('Updating contact with primary contact
			number:' + primarynumber);
			 data.firstname = contact.firstname;
			 data.lastname = contact.lastname;
			 data.title = contact.title;
			...
			contact.primarycontactnumber;
			 data.othercontactnumbers =
				 ...
				 contact.primaryemailaddress;
				  ...
				  console.log('Successfully Updated contat with
				 primary contact number: ' + primarynumber);
				  } else {
				  console.log('Error while saving contact with
				 primary contact number:' + primarynumber);
				  console.log(error);
				  }
				  });
